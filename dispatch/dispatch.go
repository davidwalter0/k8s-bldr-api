...
func ApiDispatch(unit *Request, filename string, debug bool, verbose bool) (exit int, err error, response *Response) {
	...
	if unit != nil {
		for i := 0; i < len(unit.Spec); i++ {
			...
			if len(unit.Spec[i].Send.Spec.URI) > 0 {
				...
				if len(split) == 2 {
					...
					case "consul":
						...
						rc, message = ExecuteConsulCheck(name, uri, arg0, args, expect, version, base64Encoded, verbose)
					case "https":
					...
					case "file":
						...
						rc, message = ExecuteOSCmd(name, arg0, args, expect, version, base64Encoded, verbose)
						if verbose {
							Info.Printf("ApiVersion [%8s] test [%-32s] arg0  : [%q]\n", version, name, arg0)
							Info.Printf("ApiVersion [%8s] test [%-32s] execute type [%s] cmd [%s] with args [%q]\n", version, name, typ, arg0, args)
							Info.Printf("ApiVersion [%8s] test [%-32s] rc      [%d] message[output] [%s]\n", version, name, rc, message)
						}
					default:
			...
			response.Append(name, err, rc, expect)
		}
	} else {
